                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span
                      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold ${
                        selectedRecord.net_income >= 0 ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'
                      }`}
                    >
                      {selectedRecord.net_income >= 0 ? <TrendingUp className="h-3.5 w-3.5" /> : <TrendingDown className="h-3.5 w-3.5" />}
                      {selectedRecord.net_income >= 0 ? 'Profit' : 'Loss'}
                    </span>
                  </div>
                  <p
                            ...
                            <td className="whitespace-nowrap px-6 py-4 text-sm text-slate-900">
                              <span
                                className={`inline-flex items-center gap-1.5 font-medium ${
                                  detail.net_income >= 0 ? 'text-emerald-700' : 'text-rose-700'
                                }`}
                                title={detail.net_income >= 0 ? 'Profit' : 'Loss'}
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div>
                  ...
                  >
                    <input
                      type="text"
                      value={addFormData.sales_date ? formatDisplayDate(addFormData.sales_date) : ''}
                      readOnly
                      placeholder="01 January 2026"
                      className="w-full cursor-pointer rounded-md border border-slate-300 bg-white px-3 py-2 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                    <input
                      ref={startDateRef}
                      type="date"
